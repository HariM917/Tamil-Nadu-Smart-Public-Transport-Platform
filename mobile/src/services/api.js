...
export const initAuth = async () => {
  try {
    const token = await AsyncStorage.getItem('@tn_token');
    const userStr = await AsyncStorage.getItem('@tn_user');
    if (token && userStr) {
      userToken = token;
...
export const mobileAuth = {
  ...
  setToken: async (token) => { 
    userToken = token;
    if (token) await AsyncStorage.setItem('@tn_token', token);
    else await AsyncStorage.removeItem('@tn_token');
  },
  setUser: async (user) => { 
    userData = user;
    if (user) await AsyncStorage.setItem('@tn_user', JSON.stringify(user));
    else await AsyncStorage.removeItem('@tn_user');
  },
  logout: async () => {
    userToken = null;
    userData = null;
    await AsyncStorage.removeItem('@tn_token');
    await AsyncStorage.removeItem('@tn_user');
  }
};
...
export const apiService = {
  login: async (username, password) => {
    ...
    await mobileAuth.setToken(data.access_token);
    await mobileAuth.setUser(data.user);
    return data;
  },
  ...
  getMyPasses: async () => {
    const response = await fetch(`${API_URL}/bus-pass/my-passes`, { headers: getHeaders() });
    const data = await response.json();
  ...
  searchBuses: async (source, destination) => {
    const response = await fetch(`${API_URL}/booking/search?source=${encodeURIComponent(source)}&destination=${encodeURIComponent(destination)}`, { headers: getHeaders() });
    const data = await response.json();
  ...
  getBookingHistory: async () => {
    const response = await fetch(`${API_URL}/booking/history`, { headers: getHeaders() });
    const data = await response.json();
  ...
  trackBus: async (busId) => {
    const response = await fetch(`${API_URL}/buses/track/${busId}`, { headers: getHeaders() });
    const data = await response.json();
  ...
  listBuses: async () => {
    const response = await fetch(`${API_URL}/buses/list`, { headers: getHeaders() });
    const data = await response.json();
    if (!response.ok) throw new Error(data.detail || 'Failed to fetch buses');
    return data;
  }
};
